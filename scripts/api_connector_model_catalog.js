...
function writeDocFile(file, discoveredModels, mergedIds, previousIds) {
  ...
  const lines = [];
  lines.push('# Qwen Chat Models Synchronization');
  lines.push('');
  ...
  lines.push('');
  lines.push('## Added in the latest synchronization');
  lines.push('');
  if (added.length) {
  ...
  lines.push('');
  lines.push('## Endpoint models missing from current Qwen Chat landing metadata');
  lines.push('');
  ...
  lines.push('');
  lines.push('## Final merged endpoint models list');
  lines.push('');
...
async function main() {
  ...
  console.log(`Endpoint models list saved: ${merged.length} models -> ${OUTPUT_FILE}`);
  console.log(`Synchronization report saved: ${DOC_FILE}`);
