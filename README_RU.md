...
- 🟢 **Qwen (Tongyi Qianwen)** — полная поддержка (чат, стриминг, гостевой режим).
- 🟢 **Z.ai / GLM** — Browser-backed API, поддержка Guest-режима.
- 🟢 **DeepSeek** — поддержка через захват сессий, обход Cloudflare с подменой заголовков.
- 🟢 **Kimi (Moonshot AI)** — поддержка через Browser DOM Proxy (требуется авторизация).
- 🟢 **MiniMax (Hailuo AI)** — захват сессий через официальное веб-приложение.
- 🟢 **Perplexity AI** — поддержка через копирование `__Secure-next-auth.session-token`.
- 🟢 **Xiaomi MIMO** — поддержка через Cookie сессий.
- 🟢 **HuggingFace** — использование бесплатных открытых моделей.
